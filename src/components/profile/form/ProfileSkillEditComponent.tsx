...
const ProfileSkillComponent = () => {
  ...
  const handleValidateFormSocial = () => {
    ...
    if (!REGEX_RULES.url.test(profileSocialRequest?.facebook_url)) {
      isValidForm = false;
      errorMessages.facebook_url = VALIDATE_FORM_UPDATE_PROFILE.facebook_url.format;
    }

    if (!REGEX_RULES.url.test(profileSocialRequest?.twitter_url)) {
      isValidForm = false;
      errorMessages.twitter_url = VALIDATE_FORM_UPDATE_PROFILE.twitter_url.format;
    }

    if (!REGEX_RULES.url.test(profileSocialRequest?.github_url)) {
      isValidForm = false;
