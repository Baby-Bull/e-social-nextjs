...
const BoxInfoProfile = styled(Box)`
  ...
  color: #1a2944;
`;
const CommunityOgp: React.SFC<CommunityOgpProps> = ({ community }) => (
  <Box sx={{ backgroundColor: theme.whiteBlue, padding: "33px 53px" }}>
    ...
    <Box sx={{ backgroundColor: "white", padding: "49px 40px", display: "flex" }}>
      <Box sx={{ marginRight: "30px" }}>
        <Avatar src={community?.profile_image} sx={{ width: "360px", height: "360px" }} />
      </Box>
      <Box sx={{ color: theme.navy }}>
        ...
        <BoxInfoProfile>参加人数 :{community?.member_count}人</BoxInfoProfile>
        <Box
          sx={{
