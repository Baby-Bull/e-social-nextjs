...
import { ICommunityCardItem, IStoreState } from "src/constants/interfaces";
...
const CommunityCardComponent: React.SFC<{ data: ICommunityCardItem }> = ({ data }) => {
      ...
      <Box className={styles.boxItemCommunity}>
        ...
        <Box onClick={() => router.push(`community/${data?.id}`)} sx={{ cursor: "pointer" }}>
          <div className="image-community">
            <img className="image" src={data?.profileImage ?? "/assets/images/logo/logo.png"} alt={data?.name} />
          </div>
